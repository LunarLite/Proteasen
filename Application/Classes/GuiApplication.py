...
# > object.run_without()
#
# object.specs() returns selected algorithm, sequence, dimension (and iterations)
...
class Gui_Application(tk.Tk):
	"""Creates an GUI application which can be used to select an algorithm, 
		sequence, dimension and number of iterations.
	
	Keyfuntions:
	__init__ -- creates the frames and widgets of the application.
	run() -- starts GUI application with listbox sequences.
	run_without() -- starts GUI application without listbox sequences.
	load_sequences_from_csv() -- loads sequences from csv file into array.
	load_listbox() -- adds sequences from array to listbox.
	select_from_listbox() -- enters selected sequence into entry bar.
	on_checkbutton_click() -- determines which property widgets to show for each algorithm type.
	check() -- makes sure that only one button can be checked at the same time.
	show_properties() -- shows appropriate property widgets for each algorithm type.
	hide_properties() -- hides inappropriate porperty widgets for each algorithm type.
	validate() -- checks if entries are valid, closes the GUI application if so.
	get() -- get current entries.
	specs() -- can be called after a valid run of the GUI application to get the chosen
		specifications (algorithm, sequence, dimension (and number of iterations))

	"""

	def __init__(self):
		"""Creates the frames and widgets of the GUI application."""
		...
		# checkbuttons to determine algorithm type
		var1 = tk.IntVar()
		var1.set(0)
		...
		# create frame to set grid for property checkbuttons and entry
		...
		# checkbuttons to determine whether to run with simulatedannealing
		...
		# checkbuttons to determine wheter to start with straigth or random folded chain
		...
		# checkbuttons to determine breath first pruning 
		...
		# checkbuttons to determine depth first hillclimber afterwards
	...
	def run(self, id, sequence_data):
		"""This function runs the GUI application and loads listbox with sequences 
		from a csv file or sequences directly from an array.

		Keyword arguments:
		id -- type of imput for sequence_data: array or csv.
		sequence_data -- array of sequences or csv file contain sequences.
	...
	def run_without(self):
		"""This function runs the GUI application without loading listbox
		with sequences."""
	...
	def load_sequences_from_csv(self, csv_file): 
		"""This function loads sequences from csv file into array.

		Keyword arguments:
		csv_file -- csv file containing sequences. 
			Has to be of format: sequence_id, sequence.
	...
	def load_listbox(self, sequences):
		"""This function adds sequences from array to listbox.

		Keyword arguments:
		sequences -- array with sequences.
	...
	def select_from_listbox(self, event):
		"""This function enters selected sequence into entry bar.
	...
	def on_checkbutton_click(self, checked):
		"""This function determines which property widgets 
		to show for checked algorithm type.

		Keyword arguments:
		checked -- name of selected algorithm type checkbutton.
	...
	def check(self, checked, checkbuttons):
		"""This functions checks selected checkbutton, and uncheck previous if any. 
		Makes sure that checkbuttons are not selected simultaneously. 

		Keyword arguments:
		checked -- name of selected algorithm type checkbutton.
		checkbuttons -- dictionary of checkbutton group. 
			dictionary is of format: checkbuttons["checkbutton name"] = 
				{"button": checkbuttons widget, "var": var}.
	...
	def show_properties(self, prop):
		"""This function shows group of property widgets.

		Keyword arguments:
		prop -- name of group of property widgets to show 
			(startchain/pruning/hillafter/iterations).
	...
	def hide_properties(self, prop): 
		"""This function hides group of property widgets.

		Keyword arguments:
		prop -- name of group of property widgets to hide 
			(startchain/pruning/hillafter/iterations).
	...
	def validate(self):
		"""This function checks if entries are valid and closes GUI application 
		if so.""" 
		...
		# ensure algorithm type is given
	...
	def get(self, g):
		"""This function returns current entries.

		Keyword arguments:
		g -- desired entry (algorithm_type / annealing / dimension / hillafter
			 / iterations / pruning / startwith / sequence).
	...
	def specs(self):
		"""This function returns chosen specifications (algorithm, sequence, 
			dimension (and number of iterations)), when called after a valid
			run of GUI application."""
